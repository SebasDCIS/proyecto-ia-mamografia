...
% Autores: Sebastián Inostroza Hurtado · Dr. Alejandro Veloz Baeza
...
\author{
    ...
    \thanks{\textsuperscript{1}Estudiante de Doctorado, Doctorado en Ciencias e Ingenier\'ia para la Salud, Universidad de Valpara\'iso, Valpara\'iso, Chile. (e-mail: sebastian.inostroza@postgrado.uv.cl)}
...
Este art\'iculo presenta el dise\~no, implementaci\'on y validaci\'on de un sistema local de auditor\'ia t\'ecnica de informes mamogr\'aficos en espa\~nol. La propuesta integra cuatro m\'odulos: (i) un extractor de categor\'ia BI-RADS basado en reglas robusto a variaciones tipogr\'aficas, (ii) un extractor de recomendaciones que combina normalizaci\'on, reglas cl\'inicas y similitud sem\'antica, (iii) un motor de cotejo contra una tabla normativa BI-RADS/ACR dise\~nada desde la pr\'actica cl\'inica chilena, y (iv) un verificador secundario basado en DistilBETO fine-tuneado que valida cruzadamente la extracci\'on. Toda la ejecuci\'on es local, garantizando cumplimiento de la Ley N.\textsuperscript{o}~19.628 sobre protecci\'on de datos personales en el contexto chileno previsto para su despliegue \cite{ley19628}.

Las contribuciones principales son: (i) la formalizaci\'on computacional de una tabla normativa BI-RADS/ACR dise\~nada desde la pr\'actica cl\'inica chilena y aplicable a informes mamogr\'aficos en espa\~nol gracias a la uniformidad regional del lenguaje cl\'inico-radiol\'ogico; (ii) una arquitectura h\'ibrida regex-ML interpretable que reduce la fatiga de alertas (1,33\% del corpus genera alerta cl\'inica) sin sacrificar sensibilidad; (iii) un verificador dual basado en regex priorit\'aria y DistilBETO secundario, que reduce las discrepancias t\'ecnicas espurias en un 82\%; y (iv) la evaluaci\'on emp\'irica sobre un corpus p\'ublico de 4\,357 informes en espa\~nol \cite{vazquez2025}, validada mediante validaci\'on cruzada estratificada para reportar m\'etricas honestas y libres del sesgo de selecci\'on de checkpoint.
...
La elecci\'on de este corpus responde a su disponibilidad p\'ublica y volumen sustancial, en ausencia de datasets chilenos abiertos comparables al momento de este trabajo. Esta decisi\'on prioriza la reproducibilidad de los resultados sin sacrificar relevancia cl\'inica: el espa\~nol cl\'inico-radiol\'ogico exhibe una alta uniformidad regional debido a la adopci\'on internacional del est\'andar BI-RADS/ACR, por lo que los patrones ling\"u\'isticos y categorizaciones identificados son razonablemente extrapolables a otros contextos hispanohablantes, incluidos los chilenos. El refinamiento espec\'ifico del sistema sobre informes mamogr\'aficos chilenos, mediante validaci\'on con radi\'ologos locales, queda identificado como trabajo futuro.
...
Bajo un split estratificado 80/20 \'unico, el modelo seleccionado por HuggingFace Trainer mediante \texttt{load\_best\_model\_at\_end=True} con \texttt{metric\_for\_best\_model="macro\_f1"} (step 1\,743) alcanz\'o Macro F1 = 0,9386 sobre el conjunto de prueba (872 informes). Sin embargo, dado que este esquema utiliza el mismo conjunto de prueba para la selecci\'on de checkpoint y para la evaluaci\'on final, la m\'etrica resultante puede estar ligeramente optimizada. Para obtener una evaluaci\'on libre de este sesgo se realiz\'o adicionalmente una validaci\'on cruzada estratificada 5-fold con \texttt{StratifiedKFold} (\texttt{random\_state=42}, \texttt{shuffle=True}) y aumentaci\'on textual aplicada \emph{por fold} (no globalmente) para evitar fuga de datos por variantes l\'exicas repartidas entre train y test del mismo fold. La m\'etrica honesta resultante es \textbf{Macro F1 = 0,8877 $\pm$ 0,0501}, que se reporta como m\'etrica oficial del clasificador BI-RADS basado en Transformer. La diferencia ($-0,0509$) respecto del split \'unico es consistente con lo esperado en evaluaciones libres de sesgo de checkpoint.

\subsubsection{Tabla normativa BI-RADS/ACR}
Se construy\'o una formalizaci\'on computacional del Atlas BI-RADS de la American College of Radiology \cite{acr2013}, dise\~nada desde la pr\'actica cl\'inica chilena. La tabla mapea cada categor\'ia BI-RADS (0--6) a una jerarqu\'ia de recomendaciones esperadas, equivalentes aceptables, y subcategor\'ias de notificaci\'on suave, seg\'un el conocimiento cl\'inico del autor sobre pr\'actica radiol\'ogica nacional. Por ejemplo, BI-RADS 4 acepta como recomendaci\'on esperada \emph{biopsia hist\'ologica}, con \emph{derivaci\'on oncol\'ogica} como equivalente cl\'inicamente aceptable; BI-RADS 5, en cambio, se trata de forma estricta sin equivalentes dado su criticidad cl\'inica.
...
[PENDIENTE: completar en pr\'oxima sesi\'on con los resultados detallados de los cuatro m\'odulos: extracci\'on BI-RADS Macro F1 0,9995; clasificaci\'on de recomendaciones cobertura completa; cotejo ACR con 1,33\% de alertas cl\'inicas; verificaci\'on dual con 96,4\% confirmados, 3,0\% confirmaci\'on cruzada, 0,6\% sin confirmaci\'on de ML, 0,07\% discrepancias reales.]
...
[PENDIENTE: completar con tabla de distribuci\'on de estados de verificaci\'on, an\'alisis de los casos discrepantes, y discusi\'on de las m\'etricas honestas del Transformer: (i) split \'unico Macro F1 0,9386 (con sesgo de selecci\'on de checkpoint), (ii) \textbf{CV 5-fold Macro F1 0,8877 $\pm$ 0,0501 (m\'etrica oficial)}, (iii) F1 sobre el bloque CONCLUSI\'ON aplicado en producci\'on: 0,8987 (caracter\'istica del verificador secundario, no del clasificador en s\'i, ya que el verificador analiza solo el bloque y no el informe completo). Tambi\'en se discutir\'a el trade-off de aplicar DistilBETO solo sobre la conclusi\'on, y c\'omo la CV demuestra que la ventaja sobre LinearSVC (Macro F1 0,7369 $\pm$ 0,0703) se mantiene robusta entre folds ($+2,14\sigma$ del baseline cl\'asico).]

\section{Conclusiones}
[PENDIENTE: completar en pr\'oxima sesi\'on con las principales conclusiones del trabajo, enfatizando: (i) la viabilidad del enfoque h\'ibrido regex+ML para auditor\'ia t\'ecnica de informes cl\'inicos; (ii) el aporte de la tabla normativa BI-RADS/ACR formalizada; (iii) la baja tasa de alertas espurias (0,07\% requieren revisi\'on manual); (iv) las limitaciones del verificador ML sobre clases minoritarias; (v) la importancia de la validaci\'on cruzada para reportar m\'etricas honestas en proyectos de NLP cl\'inico con desbalance de clases.]

\subsection*{Trabajo futuro}
[PENDIENTE: completar con los puntos espec\'ificos: (i) validaci\'on cl\'inica formal con radi\'ologos chilenos sobre las alertas generadas; (ii) re-evaluaci\'on del sistema sobre corpus chilenos cuando est\'en disponibles; (iii) refinamiento de la tabla normativa con retroalimentaci\'on de pr\'actica cl\'inica local; (iv) implementaci\'on de b\'usqueda sem\'antica h\'ibrida (regex amplia + ranking ML) para reducir la dependencia del verificador secundario de la presencia de bloque CONCLUSI\'ON identificable; (v) extensi\'on del pipeline a informes de ecograf\'ia mamaria; (vi) integraci\'on con sistemas RIS/PACS hospitalarios; (vii) dashboard interactivo para visualizaci\'on de alertas en flujos asistenciales (en desarrollo).]
