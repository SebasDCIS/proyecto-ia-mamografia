...
La distribuci\'on de etiquetas est\'a marcadamente desbalanceada: BI-RADS 2 concentra 2\,635 informes (60,5\%), seguido de BI-RADS 0 (966, 22,2\%) y BI-RADS 1 (596, 13,7\%). Las clases cl\'inicamente cr\'iticas son minoritarias: BI-RADS 4 (52 informes, 1,2\%), BI-RADS 3 (87, 2,0\%), BI-RADS 5 (16, 0,4\%) y BI-RADS 6 (5, 0,1\%). Este desbalance motiv\'o el uso de Macro F1 como m\'etrica principal~\cite{sokolova2009systematic} y la aplicaci\'on de aumentaci\'on textual durante el entrenamiento del modelo Transformer.

\subsubsection{Modelo Transformer y protocolo de evaluaci\'on}
Se utiliz\'o DistilBETO (\texttt{dccuchile/distilbert-base-spanish-uncased}) \cite{canete2020}, una versi\'on destilada de BETO, que a su vez adapta la arquitectura BERT~\cite{devlin2019bert} basada en Transformers~\cite{vaswani2017attention}, siguiendo el esquema DistilBERT \cite{sanh2019}, como modelo base, fine-tuneado sobre el corpus de entrenamiento con tres \'epocas, batch size 8, learning rate $2\times10^{-5}$, y aumentaci\'on textual sobre clases minoritarias mediante diccionarios de sin\'onimos cl\'inicos. La elecci\'on de DistilBETO sobre alternativas como BETO completo o XLM-RoBERTa se justifica por su menor huella computacional (\textasciitilde 250 MB), su entrenamiento espec\'ifico en espa\~nol y la disponibilidad de aceleraci\'on local mediante Metal Performance Shaders (MPS) de Apple Silicon.

Bajo un split estratificado 80/20 \'unico, el mejor modelo alcanz\'o Macro F1 = 0,9386 sobre el conjunto de prueba (872 informes). Este esquema, sin embargo, usa el mismo conjunto de prueba para seleccionar el punto de control y para la evaluaci\'on final, por lo que la m\'etrica resultante puede quedar algo optimizada. Para obtener una evaluaci\'on libre de ese sesgo se realiz\'o adem\'as una validaci\'on cruzada estratificada de 5 particiones~\cite{kohavi1995study} (con semilla fija), aplicando la aumentaci\'on textual dentro de cada partici\'on y no de forma global, de modo que las variantes l\'exicas de un mismo informe no se repartieran entre entrenamiento y prueba. La m\'etrica honesta resultante es \textbf{Macro F1 = 0,8877 $\pm$ 0,0501}, que es la del clasificador finalmente descartado. La Fig.~\ref{fig:folds} muestra el desempe\~no por partici\'on: la ca\'ida del fold~4 a 0,80 refleja la sensibilidad del Macro F1 a las clases minoritarias y es la raz\'on de reportar la validaci\'on cruzada en lugar de una partici\'on \'unica, cuyo resultado depender\'ia de qu\'e informes cayeron en el conjunto de prueba (Secci\'on~\ref{sec:verif}). La diferencia respecto del split \'unico ($-0,0509$) es la esperable al eliminar el sesgo de selecci\'on.
...
La versi\'on en uso (\codename{buscador\_birads.py}) elimina esa dependencia con una b\'usqueda en cuatro fases, resumida en el Algoritmo~\ref{alg:buscador}. La primera recorre el informe completo y recoge todas las menciones. La segunda descarta por contexto las hist\'oricas, las comparativas, las educacionales y las negadas, atendiendo a las palabras que las preceden. La tercera pondera las restantes por su posici\'on relativa: la categor\'ia definitiva aparece al final del informe, y sobre el corpus la \'ultima menci\'on se ubica en el percentil 0,85 del texto en el 100\,\% de los casos. La cuarta reserva un arbitraje para los empates, que sobre 4\,357 informes no llegan a producirse porque el 99,9\,\% declara una sola menci\'on.
...
\begin{algorithm}[!t]
\caption{B\'usqueda h\'ibrida de la categor\'ia BI-RADS}
\label{alg:buscador}
\begin{algorithmic}[1]
\Require Informe $T$ como texto plano
\Ensure Categor\'ia $c$ con su nivel de confianza, o alerta
\State $M \gets$ todas las menciones de BI-RADS en $T$ \Comment{Fase 1}
\If{$M = \emptyset$}
  \If{$T$ contiene hallazgos o recomendaciones}
    \State \Return alerta de omisi\'on \Comment{no se imputa categor\'ia}
  \Else
    \State \Return sin categor\'ia
  \EndIf
\EndIf
\ForAll{$m \in M$} \Comment{Fase 2: filtrado contextual}
  \State $v \gets$ ventana previa a $m$
  \If{$v$ indica menci\'on hist\'orica, comparativa, educacional o negada}
    \State marcar $m$ como descartada
  \EndIf
\EndFor
\State $M' \gets \{m \in M : m$ no descartada$\}$
\If{$M' = \emptyset$}
  \State $M' \gets M$ \Comment{repliegue: ninguna sobrevive al filtro}
\EndIf
\ForAll{$m \in M'$} \Comment{Fase 3: ponderaci\'on posicional}
  \State $p(m) \gets \mathrm{pos}(m) / |T|$
  \State $w(m) \gets \alpha \cdot p(m) + \beta \cdot \mathrm{especificidad}(m)$
\EndFor
\State $C \gets \arg\max_{m \in M'} w(m)$
\If{$|C| > 1$} \Comment{Fase 4: arbitraje solo si hay empate}
  \State $C \gets$ desempate por posici\'on m\'as tard\'ia
\EndIf
\State \Return categor\'ia de $C$ con confianza seg\'un el patr\'on que la produjo
\end{algorithmic}
\end{algorithm}
...
El Algoritmo~\ref{alg:cotejo} resume la l\'ogica completa. La auditor\'ia de coherencia es binaria: una recomendaci\'on es coherente o no lo es. Es coherente cuando coincide con la esperada, es un equivalente aceptable, o es m\'as cauta que la m\'inima esperada, ya que un exceso de precauci\'on --por ejemplo, biopsiar un BI-RADS 3-- es cl\'inicamente v\'alido. Es incoherente cuando la recomendaci\'on se queda corta frente a la norma. Toda incoherencia genera una alerta; lo que var\'ia es su severidad, no el hecho de constituir o no un problema. As\'i, la palabra alerta conserva su significado, y su gravedad, desde una incoherencia leve y sin riesgo hasta una cr\'itica, comunica cu\'anto importa.
...
\begin{algorithm}[!t]
\caption{Cotejo de coherencia BI-RADS/ACR}
\label{alg:cotejo}
\begin{algorithmic}[1]
\Require Categor\'ia $b$, recomendaci\'on clasificada $r$, tabla normativa $\mathcal{T}$
\Ensure Estado, severidad y regla aplicada
\If{$r$ es nula o ambigua} \Comment{abstenci\'on: no se juzga coherencia}
  \If{$b \in \{4,5,6\}$}
    \State \Return \textsc{revisi\'on}, severidad \textbf{cr\'itica}
    \Statex \hfill \emph{la norma exige conducta expl\'icita para estas categor\'ias}
  \Else
    \State \Return \textsc{revisi\'on}, severidad media
  \EndIf
\EndIf
\State $e \gets \mathcal{T}[b].\text{esperada}$
\If{$r = e$}
  \State \Return \textsc{coherente}
\ElsIf{$r \in \mathcal{T}[b].\text{equivalentes}$}
  \State \Return \textsc{coherente\_equivalente}
\ElsIf{$\mathrm{urgencia}(r) > \mathrm{urgencia}(e)$}
  \State \Return \textsc{coherente\_con\_precauci\'on}
  \Statex \hfill \emph{un exceso de cautela es cl\'inicamente v\'alido}
\EndIf
\State $d \gets \mathrm{urgencia}(e) - \mathrm{urgencia}(r)$ \Comment{desv\'io en la jerarqu\'ia}
\If{$b \in \{4,5\}$ \textbf{y} $r \notin \{\text{biopsia}, \text{derivaci\'on}\}$}
  \State \Return \textsc{incoherente}, severidad \textbf{cr\'itica}
  \Statex \hfill \emph{sospecha sin acci\'on diagn\'ostica}
\EndIf
\State severidad $\gets$ funci\'on de $\mathrm{criticidad}(b)$ y de $d$
\State \Return \textsc{incoherente}, severidad
\end{algorithmic}
\end{algorithm}
...
\begin{tabular}{@{}p{2.7cm}p{2.5cm}r@{}}
\toprule
M\'odulo & M\'etrica & Valor \\
\midrule
Extracci\'on BI-RADS (reglas) & Macro F1 / cobertura & 0,9995 / 100\,\% \\
Clasificaci\'on de la recomendaci\'on & Cobertura & 99,82\,\% \\
